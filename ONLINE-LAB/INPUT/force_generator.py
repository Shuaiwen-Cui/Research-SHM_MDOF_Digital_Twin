...
class ForceGenerator:
    ...
    def __init__(self, channels: CommunicationChannels, 
                 ...
                 pulse_amplitude: float = 500.0,
                 use_pulses: bool = False):
        """
        Parameters:
        -----------
        ...
        noise_std: float
            Standard deviation of white noise (N) - reduced from 100.0 to 10.0
        pulse_interval: float
            Time interval between pulses (seconds) - only used if use_pulses=True
        pulse_duration: float
            Duration of each pulse (seconds) - only used if use_pulses=True
        pulse_amplitude: float
            Amplitude of pulse load (N) - if None, will be calculated based on peak acceleration (2-10 g)
            Default: calculated as mass × 5g (average of 2-10g range)
        use_pulses: bool
            Whether to use periodic pulses (False = pure Gaussian white noise, matching SYSID)
        """
        ...
        # Calculate pulse amplitude based on peak acceleration if not provided
        # Impact peak acceleration: 2-10 g, default use 5g (average)
        if pulse_amplitude is None:
            # Use average mass for calculation
            avg_mass = np.mean(m_initial)
            # Default to 5g (average of 2-10g range)
            default_g = 5.0
            pulse_amplitude = avg_mass * default_g * 9.81  # Force = mass × acceleration
    ...
    def trigger_earthquake(self):
        ...
        # Normalize and rescale based on peak acceleration in g units
        # Earthquake peak acceleration: 2-3 g (19.62 - 29.43 m/s²)
        # Force = mass × acceleration, so peak force = m × (2-3) × g
        PGA = max(abs(Ag))
        if PGA > 0:
            # Random peak acceleration between 2-3 g
            peak_g = float(np.random.rand(1) * 1.0 + 2.0)  # 2.0 - 3.0 g (convert to scalar)
            peak_acceleration = peak_g * 9.81  # Convert to m/s²
            rescale_factor = peak_acceleration
        ...
        peak_force = np.max(m_initial) * max(abs(earthquake_acc))
        print(f"Earthquake event triggered, duration: {len(earthquake_acc)*dt:.2f}s (PGA: {max(abs(earthquake_acc)):.2f} m/s², {max(abs(earthquake_acc))/9.81:.2f}g, peak force: {peak_force:.0f} N)")
    ...
    def trigger_strongwind(self):
        ...
        # Apply scaling factor to make wind load visible relative to white noise (10.0 N)
        # Target: wind force should be 20-50x white noise std (200-500 N per DOF)
        # Increased from 0.001 to 0.1 (100x increase)
        ...
        max_wind_force = np.max(np.abs(strongwind_load))
        print(f"Strong wind event triggered, duration: {len(strongwind_load[0])*dt:.2f}s (max force: {max_wind_force:.1f} N, scaled by {wind_scale_factor})")
    ...
    def generate_force(self) -> np.ndarray:
        ...
        Matching SYSID: pure Gaussian white noise (no pulses by default)
        """
        force = np.zeros(nDOF)
        
        # Always add white noise (can be superimposed with other events)
        # Matching SYSID: pure Gaussian white noise
        ...
        # Manual impacts can always be triggered (even if use_pulses=False)
        # Auto pulses only work if use_pulses=True
        if self.use_pulses and self.auto_mode:
            # Auto mode: periodic pulses
            if self.current_time >= self.next_pulse_time:
                # Start new pulse
                # Calculate impact force based on peak acceleration: 2-10 g
                ...
                # Random peak acceleration between 2-10 g
                peak_g = float(np.random.rand(1) * 8.0 + 2.0)  # 2.0 - 10.0 g (convert to scalar)
                peak_acceleration = peak_g * 9.81  # Convert to m/s²
                # Calculate force: F = m × a
                self.current_pulse_amplitude = m_initial[self.pulse_dof] * peak_acceleration
                
                self.pulse_end_time = self.current_time + self.pulse_duration
                self.next_pulse_time = self.current_time + self.pulse_interval
        
        # Manual impact trigger (works regardless of use_pulses setting)
        if self.manual_trigger:
            ...
            peak_acceleration = peak_g * 9.81  # Convert to m/s²
            # Calculate force: F = m × a
            impact_amplitude = m_initial[self.pulse_dof] * peak_acceleration
            
            self.pulse_end_time = self.current_time + self.pulse_duration
            self.manual_trigger = False
            # Store the amplitude for this pulse
            self.current_pulse_amplitude = impact_amplitude
            print(f"Manual impact triggered (peak: {peak_g:.2f}g, acceleration: {peak_acceleration:.1f} m/s², force: {impact_amplitude:.0f} N, DOF: {self.pulse_dof+1}, direction: {'left' if self.pulse_direction == -1 else 'right'})")
        elif not self.pulse_active and not self.use_pulses:
            # Reset pulse amplitude to default if no active pulse and not in auto mode
